...
const UserManagementTable = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/users?search=${searchTerm}`, {
  ...
  const handleUpdateRole = async (id, newRole) => {
    const token = localStorage.getItem('token');
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/update-role/${id}`, { role: newRole }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchUsers();
    } catch (err) {
