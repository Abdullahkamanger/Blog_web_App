...
const BlogManagementHub = () => {
  ...
  const fetchBlogs = async () => {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/all-blogs`, {
  ...
  const fetchChartData = async () => {
    const token = localStorage.getItem('token');
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/blog-stats-chart`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const handlePreview = async (id) => {
    const token = localStorage.getItem('token');
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/blog-preview/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const togglePublish = async (id, currentStatus) => {
    const token = localStorage.getItem('token');
    await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/toggle-publish/${id}`, { status: !currentStatus }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    fetchBlogs();
  };
