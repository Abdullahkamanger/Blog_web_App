...
const AuthorApprovalTable = () => {
  ...
  const loadPending = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/pending-authors`, {
  ...
  const handleApprove = async (id) => {
    const token = localStorage.getItem('token');
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/approve-author/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
